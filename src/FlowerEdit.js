...
class FlowerEdit extends Component {
    ...
    async componentDidMount() {
        ...
        if (this.props.match.params.plant_id !== 'new') {
            const flowergarden = await (await fetch(`http://localhost:8080/FlowerGarden/${this.props.match.params.plant_id}`)).json();
    ...
    async handleSubmit(event) {
        ...
        await fetch('http://localhost:8080/FlowerGarden' + (item.plant_id ? '/' + item.plant_id : ''), {
        ...
        this.props.history.push('http://localhost:8080/FlowerGarden');
    }
    render() {
        const {item} = this.state;
        ...
        return <div>
            ...
            <Container>
                ...
                <Form onSubmit={this.handleSubmit}>
                    ...
                    <Button color="secondary" tag={Link} to="http://localhost:8080/FlowerGarden">Cancel</Button>
                </FormGroup>
